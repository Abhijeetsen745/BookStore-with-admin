...
function UserOrderHIstory() {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/getOrderHistory`, {
          headers,
        });
  ...
  return(
    <>
    {!orderHistory &&  <div className="flex justify-center items-center h-screen bg-zinc-900">
          <Loader />
        </div>}
      { orderHistory  && orderHistory.length === 0 && (
        <div className="h-screen">
          <div className="h-[100%] flex items-center justify-center flex-col">
            ...
            <img src="https://cdn-icons-png.flaticon.com/128/9961/9961218.png" alt="" className="lg:h-[50vh] mb-8" />
          </div>
        </div>
      )}
      {orderHistory && orderHistory.length>0 && (
